...
export default function Navbar() {
  ...
  // Compute active index from location
  useEffect(() => {
    ...
    <nav className="sticky top-0 z-50 bg-primary shadow-lg">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Currency selector */}
          <div className="hidden md:block relative" data-currency-dropdown>
            <button
                ...
                {availableCurrencies.map((c) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-3 hover:bg-gray-50 transition ${
                      currency.code === c.code ? 'bg-accent/10 text-accent font-semibold' : 'text-gray-700'
                    }`}
                  >
                    <span className="w-8 h-5 rounded text-xs font-bold bg-gray-100 flex items-center justify-center text-gray-600">{c.countryCode}</span>
                    <span>{c.name}</span>
                  </button>
                ))}
              </motion.div>
          ...
          >
            <div className="px-4 py-3 flex flex-col gap-1">
              ...
              <div className="mt-2 pt-2 border-t border-white/10">
                <p className="text-xs text-gray-500 px-4 mb-1">Devise</p>
